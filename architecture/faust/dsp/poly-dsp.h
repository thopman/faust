...
class mydsp_poly : public dsp_voice_group, public dsp_poly {
    ...
    private:
        ...
        bool stealPrefer(int a, int b)
        {
            ...
            switch (fPolicy.steal) {
                case StealMode::QUIETEST: return va->fLevel  < vb->fLevel;
                case StealMode::LOWEST:   return va->fCurNote < vb->fCurNote;
                case StealMode::HIGHEST:  return va->fCurNote > vb->fCurNote;
                case StealMode::OLDEST:
                default:                  return va->fDate    < vb->fDate;
            }
        }
        ...
        int soundingPitch(size_t i)
        {
            ...
            if (cur >= 0) return cur;                                 // active note
            if (cur == kLegatoVoice) return fVoiceTable[i]->fNextNote; // becoming active
                ...
                switch (fPolicy.priority) {
                    case NotePriority::LOW:
                        std::sort(sel.begin(), sel.end(),
                                  [](const NoteInfo& a, const NoteInfo& b) { return a.fPitch < b.fPitch; });
                        break;
                    ...
                    default:
                        std::sort(sel.begin(), sel.end(),
                                  [](const NoteInfo& a, const NoteInfo& b) { return a.fDate > b.fDate; });
                        break;
                }
        ...
        void updateVoices()
        {
            ...
            auto inTarget = [&](int pitch) {
                for (const NoteInfo& n : target) if (n.fPitch == pitch) return true;
                return false;
            };
            auto isSounding = [&](int pitch) {
                for (size_t i = 0; i < fVoiceTable.size(); i++) if (soundingPitch(i) == pitch) return true;
                return false;
            };
            ...
            for (size_t i = 0; i < fVoiceTable.size(); i++) {
                ...
                if (p >= 0 && !inTarget(p)) dropped.push_back(int(i));
            }
            // Target notes not yet sounding.
            std::vector<NoteInfo> added;
            for (const NoteInfo& n : target) {
                if (!isSounding(n.fPitch)) added.push_back(n);
            }
            ...
            for (const NoteInfo& n : added) {
                int v = getFreeVoice();
                fVoiceTable[v]->keyOn(n.fPitch, n.fVelocity, fVoiceTable[v]->fCurNote == kLegatoVoice);
            }
        }
    ...
    public:
        ...
        mydsp_poly(::dsp* dsp,
                   ...
                   bool control = false,
                   bool group = true,
                   const PolyPolicy& policy = PolyPolicy())
        : dsp_voice_group(panic, this, control, group), dsp_poly(dsp) // dsp parameter is deallocated by ~dsp_poly
        {
            fDate = 0;
            fMidiHandler = nullptr;

            // Note-allocation policy: derive the simultaneously-sounding budget from
            // the voicing mode, but never exceed the physical voice pool.
            fPolicy = policy;
            switch (fPolicy.voicing) {
                case Voicing::MONO: fMaxSounding = 1; break;
                case Voicing::DUO:  fMaxSounding = 2; break;
                default:            fMaxSounding = nvoices; break;
            }
        ...
        {
            return new mydsp_poly(fDSP->clone(), int(fVoiceTable.size()), fVoiceControl, fGroupControl, fPolicy);
        }
...
struct dsp_poly_factory : public dsp_factory {
     ...
     * @param policy - note-priority / trigger / steal configuration. Defaults reproduce the historical
     *                behavior; the per-DSP '[note_priority][trigger][steal][voicing]' options metadata
                                    ...
                                    PolyPolicy policy = PolyPolicy())
    {
        // Read 'nvoices' (when -1) and the note-allocation policy from the DSP metadata.
        {
            ...
            if (nvoices == -1) nvoices = meta_nvoices;
            delete dsp;
        }
        dsp_poly* dsp_poly = new mydsp_poly(adaptDSP(fProcessFactory->createDSPInstance(), is_double), nvoices, control, group, policy);
        if (fEffectFactory) {
